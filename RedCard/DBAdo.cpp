...
bool CDBAdo::CreateInstance()
{
	...
	if(m_ptrCommand==NULL)
	{
		m_strErrorMsg	= "数据库命令对象创建失败";
		return	false;
	}
	if(m_ptrRecordset==NULL)
	{
		m_strErrorMsg	= "数据库记录集对象创建失败";
		return	false;
	}
	if(m_ptrConnection==NULL)
	{
		m_strErrorMsg	= "数据库连接对象创建失败";
		return	false;
	}
...
void CDBAdo::RecordErrorMsg(_com_error comError)
{
	...
	sprintf(error,"ADO 错误：0x%8x，%s",comError.Error(),(LPCTSTR)bstrDescribe);
	m_strErrorMsg.clear();
	m_strErrorMsg.append(error);
}

bool CDBAdo::SetConnectionString(const char* strDriver, const char* strIP, WORD wPort, const char* strCatalog, const char* strUserID, const char* strPassword)
{
	char connectionString[200] = {0};
	memset(connectionString,0,sizeof(connectionString));

	sprintf(connectionString,"Provider=%s;Data Source=%s,%ld;Network Library=DBMSSOCN;Initial Catalog=%s;User ID=%s;Password=%s;",strDriver,strIP,wPort,strCatalog,strUserID,strPassword);
	m_strConnect.clear();
	m_strConnect.append(connectionString);
	
	return true;
}

bool CDBAdo::SetConnectionString(const char* connectionString)
{	
	m_strConnect.clear();
	m_strConnect.append(connectionString);
...
bool CDBAdo::OpenConnection()
{
	try
	{
		...
		//连接数据库
		DetectResult(m_ptrConnection->Open(_bstr_t(m_strConnect.c_str()), "", "", adModeUnknown));
